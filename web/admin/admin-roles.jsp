...
<html lang="vi">
    <head>
        ...
        <title>Phân quyền tài khoản — Admin | Booky Mart</title>
...
<link rel="stylesheet" href="${pageContext.request.contextPath}/assets/css/main.css">
    ...
    <div class="admin-shell">
        <div class="admin-sidebar">
            ...
            <a href="${pageContext.request.contextPath}/admin/roles" class="active"><svg class="icon"><use href="#ic-shield"/></svg> Phân quyền tài khoản</a>
        ...
        <div class="admin-main">
            <div class="admin-topbar">
                <div style="font-weight:700;color:var(--ink);">Phân quyền tài khoản</div>
            ...
            <div class="admin-content">
                <div class="page-title-row">
                    <div>
                        ...
                        <div class="sub">Cập nhật vai trò (role) của tài khoản trong hệ thống (UC-ADM-01)</div>
                    </div>
                </div>

                <div class="admin-toolbar">
                    <form class="admin-search-box" method="get" action="${pageContext.request.contextPath}/admin/roles">
                        ...
                        <form method="get" action="${pageContext.request.contextPath}/admin/roles">
                ...
                <table class="table-shop">
                    <thead><tr><th>Họ tên</th><th>Email</th><th>Vai trò hiện tại</th><th>Đổi thành</th><th></th></tr></thead>
                    <tbody>
                        <c:forEach var="acc" items="${accounts}">
                        <form method="post" action="${pageContext.request.contextPath}/admin/roles" class="role-form" id="roleForm${acc.accountId}">
                            <tr>
                            ...
                            <td>
                                <select name="newRole" id="newRole${acc.accountId}" class="form-select-shop" style="padding:7px 10px;font-size:.82rem;max-width:160px;" ${acc.role == 'ADMIN' ? 'disabled' : ''}>
                            ...
                            <td>
                                <c:choose>
                                    <c:when test="${acc.role == 'ADMIN'}">
                                        <button type="button" class="btn-sm-shop btn-outline-ink" disabled>Cập nhật</button>
                                    </c:when>
                                    <c:otherwise>
                                        <button type="button" class="btn-sm-shop btn-terracotta"
                                                onclick="confirmRoleChange('roleForm${acc.accountId}', 'newRole${acc.accountId}', '${acc.fullName}', '${acc.role}')">Cập nhật</button>
                                    </c:otherwise>
                                </c:choose>
                            </td>
                            </tr>
                    ...
                    <c:if test="${empty accounts}">
                        <tr><td colspan="5" class="text-center" style="color:var(--ink-soft);padding:24px 0;">Chưa có tài khoản nào.</td></tr>
                    </c:if>
                    </tbody>
                </table>
                ...
                <p class="form-hint mt-3">
                    ...
                    Chỉ Admin có quyền đổi vai trò. Không thể gán vai trò Admin cho tài khoản khác qua màn hình này. Manager không thể tự cấp quyền cho mình hoặc người khác (BR-ADM03.2).
                </p>

                <c:set var="baseUrl" value="${pageContext.request.contextPath}/admin/roles" scope="page"/>
                <%@ include file="/WEB-INF/pagination.jspf" %>

                <!-- Modal xác nhận đổi role -->
                <div class="modal-confirm-backdrop" id="confirmRoleModal">
                    <div class="modal-confirm-box">
                        <h3>Xác nhận đổi vai trò?</h3>
                        <p id="confirmRoleText"></p>
                        <div class="d-flex gap-2">
                            <button class="btn-outline-ink btn-block-shop" onclick="document.getElementById('confirmRoleModal').classList.remove('open')">Hủy</button>
                            <button class="btn-terracotta btn-block-shop" id="confirmRoleSubmitBtn">Xác nhận</button>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        function confirmRoleChange(formId, selectId, fullName, currentRole) {
            var newRole = document.getElementById(selectId).value;
            if (newRole === currentRole) {
                document.getElementById(formId).submit();
                return;
            }
            pendingFormId = formId;
            document.getElementById('confirmRoleText').innerHTML =
                    'Tài khoản <strong>' + fullName + '</strong> sẽ chuyển từ <strong>' + currentRole +
                    '</strong> sang <strong>' + newRole + '</strong>. Hành động này sẽ được ghi vào nhật ký hoạt động.';
            document.getElementById('confirmRoleModal').classList.add('open');
        }

        document.getElementById('confirmRoleSubmitBtn').addEventListener('click', function () {
            if (pendingFormId) {
